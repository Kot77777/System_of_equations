...
template<typename T>
Vector<T> method_Jacobi(const CSR_matrix<T> &A, const Vector<T> &b, const Vector<T> &x_0, const std::size_t N_iter,
                        const T eps) {
    Vector<T> x_i{x_0};
    std::size_t n_iter{};
    T d_ii{};
    while (!cond_stop(A, b, x_i, eps) && n_iter < N_iter) {
        for (std::size_t i = 0; i != A.N(); ++i) {
            ...
            for (std::size_t k = k_start; k < k_end; ++k) {
                if (A.cols(k) != i) { x_i(i) += A.values(k) * x_i(A.cols(k)); } else { d_ii = A.values(k); }
            }
            x_i(i) = (b(i) - x_i(i)) / d_ii;
        }
        n_iter += 1;
    }
    return x_i;
